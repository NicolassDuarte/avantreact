...
const fields = [
  { key: "nome", label: "Nome do item", icon: <FaBoxOpen style={{ marginRight: 14, color: "#4caf50", fontSize: 24 }} />, type: "text" },
  { key: "tipo", label: "Tipo", icon: <RiFileSearchFill style={{ marginRight: 14, color: "#4caf50", fontSize: 24 }} />, type: "select" },
  { key: "descricao", label: "Descrição", icon: <MdDescription style={{ marginRight: 14, color: "#4caf50", fontSize: 24 }} />, type: "textarea" },
  { key: "localizacao", label: "Localização", icon: <FaMapMarkerAlt style={{ marginRight: 14, color: "#4caf50", fontSize: 24 }} />, type: "localizacao" },
  { key: "data", label: "Data", icon: <FaCalendarAlt style={{ marginRight: 14, color: "#4caf50", fontSize: 24 }} />, type: "date" },
];
...
const CadastroProduto = () => {
  ...
  const openPopup = (field) => {
    if (field.key === "localizacao") {
      setLocalizacaoTemp(values.localizacao || { cep: "", rua: "", numero: "", bairro: "", cidade: "", estado: "" });
      setCepError("");
      setPopup({ open: true, field, tempValue: "" });
    } else {
      setPopup({ open: true, field, tempValue: values[field.key] || "" });
    }
  };
          ...
          <div className="col-12 col-sm-10 col-md-8 col-lg-6 col-xl-5">
            <div className="cadastro-card">
              ...
              <form onSubmit={handleSubmit}>
                {fields.map((field) => (
                  <div key={field.key} className="mb-3 d-flex align-items-center cadastro-field" onClick={() => openPopup(field)}>
                    {field.icon}
                    <div className="cadastro-field-value">
                      {field.key === "localizacao"
                        ? values.localizacao?.cep
                        ...
                        : field.key === "tipo"
                          ...
                          : values[field.key]
                            ? (field.type === "date" ? limitarTexto(new Date(values[field.key]).toLocaleDateString("pt-BR")) : limitarTexto(values[field.key]))
                            : <span className="cadastro-placeholder">{field.label}</span>}
                    </div>
                  </div>
                ))}
